(ns event-sourcing.migrations-cli
  "CLI helpers for running schema migrations against a configured database.
   ...
   - DB_USER
   - DB_PASSWORD"
  (:require [event-sourcing.migrations :as migrations]
            [next.jdbc :as jdbc]))
    ...
    (if (contains? known-commands command)
      ...
      (do
        (binding [*out* *err*]
          ...
          (println "Usage: clj -M -m event-sourcing.migrations-cli [migrate|rollback|status]"))
        (*exit-fn* 1)))))
