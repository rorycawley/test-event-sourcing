...
(defn make-event-validator
  "Returns a function that upcasts + validates one domain event map.
   event-schemas: {[\"event-type\" version] -> malli-schema}
     Value is the **payload** schema only — the envelope fields
     (event-type, event-version, payload) are wrapped automatically.
   upcast-fn:     output of make-event-upcaster"
  [event-schemas upcast-fn]
  (let [full-schemas
        (into {}
              (map (fn [[[etype ever] payload-schema]]
                     [[etype ever]
                      [:map
                       [:event-type [:= etype]]
                       [:event-version [:= ever]]
    ...
    (fn [event]
      (let [event  (upcast-fn event)
            schema (get full-schemas [(:event-type event) (:event-version event)])]
        (when-not schema
