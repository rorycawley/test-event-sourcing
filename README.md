...
Implements a bank account domain and a notification module to demonstrate: append-only event storage, optimistic concurrency with retry, command idempotency, event versioning with upcasting, data-driven projections, cross-aggregate saga coordination (fund transfers), transactional outbox publishing, async CQRS projections to a separate read database, inbox-based exactly-once consumers, inter-module reactors, and BM25 full-text search via ParadeDB.

## Architecture

The system supports two modes: **synchronous** (single database, direct projection) and **async** (separate databases, RabbitMQ-driven projections and consumers). Both share the same pure domain logic.
...
Separate databases for writes and reads. Events flow through a transactional outbox and RabbitMQ to independent async projectors and consumers. Each projector owns its queue and writes to its target database.
...
```
  ...
  +-------------+                                |
                               ...
                               | Projector| | Projector| | Consumer    |
                               +----+-----+ +----+-----+ +---+--------+
                                    |             |           |
                               +----+-------------+---+ +----+--------+
                               |  Read Store (Postgres)| | Notif. DB  |
                               +-----------------------+ +------------+
```
...
The notification module (`notification.*`) demonstrates how independent modules consume integration events from other domains:

- **`notification.notification`** -- Notification Decider with lifecycle: `pending -> sent` (or `-> failed` after 3 retries, `-> abandoned`)
- **`notification.reactor`** -- Subscribes to bank integration events (via RabbitMQ) and issues `:request-notification` commands into the shared event store. The reactor bridges modules without depending on bank internals.
- **`notification.notification-projection`** -- Writes notification read models to a separate notification database
- **`notification.component`** -- ConsumerGroup with worker pool, inbox-based dedup, catch-up timer, and DLQ

**Integration event contract**: `events.bank` defines shared Malli schemas for integration events (`bank.account-opened`, `bank.money-deposited`, etc.). Both the producer (`bank.integration-events`) and consumer (`notification.reactor`) validate against this contract.

**Inbox-based exactly-once processing** (`es.consumer`): each consumer worker claims events via `INSERT ON CONFLICT DO NOTHING` into a `consumer_inbox` table. First worker wins; duplicates are silently skipped. This provides exactly-once semantics with multiple competing workers.
...
Infrastructure is managed by Stuart Sierra's [Component](https://github.com/stuartsierra/component) library. All component records live in `es.component` (domain-agnostic); system assembly lives in `bank.components`, `notification.component`, and `system` (domain-specific).

| Component | Depends on | Purpose |
|---|---|---|
| `Datasource` | -- | Postgres connection (Testcontainers or JDBC URL, HikariCP pooling) |
| `Migrator` | `Datasource` | Runs Migratus migrations on start |
| `RabbitMQConnection` | -- | RabbitMQ connection (Testcontainers, URI, or direct) |
| `SearchIndex` | `Datasource` | Ensures BM25 indexes exist on startup |
...
| `ConsumerGroup` | `EventStoreDatasource`, `TargetDatasource`, `RabbitMQ` | Worker pool with inbox dedup, catch-up timer, DLQ |
...
;; Full async (3x Postgres + RabbitMQ, outbox poller, projectors, notification consumer)
(system/full-system)
...
(system/full-system
  {:event-store     {:mode :jdbc-url :jdbc-url "..." :user "..." :password "..."}
   ...
   :notification-db {:mode :jdbc-url :jdbc-url "..." :user "..." :password "..."}
   :rabbitmq        {:mode :uri :uri "amqp://..."}})
```

## Project Structure

The codebase is split into three layers: the reusable **framework** (`es.*`), the **domain modules** (`bank.*`, `notification.*`, `events.*`), and the **composition root** (`system`).
...
|   +-- consumer.clj             #   Inbox-based exactly-once consumer (worker pool, dedup)
...
|   +-- integration_events.clj   #   Domain -> integration event mapper (outbox filter)
|   +-- system.clj               #   Synchronous composition root (single-DB wiring)
|   +-- components.clj           #   Bank module component assembly
|
+-- notification/                # Notification domain module
|   +-- notification.clj         #   Notification Decider (request/send/fail/abandon)
|   +-- reactor.clj              #   Integration event consumer -> notification commands
|   +-- notification_projection.clj  # Notification read model handlers
|   +-- component.clj            #   ConsumerGroup wiring (workers, inbox, DLQ)
|
+-- events/                      # Shared integration event contracts
|   +-- bank.clj                 #   Bank integration event Malli schemas
|
+-- system.clj                   # Full async composition root (all modules, all infra)
...
|   +-- async_projection_test.clj    # Async projection tests (2x Postgres, batching)
|   +-- outbox_test.clj          #   Outbox record, poll, and publish tests (DB)
|   +-- consumer_test.clj        #   Consumer inbox, dedup, worker pool tests
...
+-- notification/
|   +-- notification_test.clj    #   Notification Decider unit tests
+-- system_integration_test.clj  #   Full system integration test (all modules)
...
|   +-- 001-schema.{up,down}.sql
+-- notification-migrations/     # Notification DB migrations (separate DB)
    +-- 001-schema.{up,down}.sql
...
event_outbox (id BIGSERIAL PK, global_sequence BIGINT UNIQUE,
              message JSONB, published_at TIMESTAMPTZ)
...
### Notification Database (separate database, async mode)

```sql
consumer_inbox (consumer_group TEXT, global_sequence BIGINT,
               processed_at TIMESTAMPTZ)
  PK (consumer_group, global_sequence)

consumer_checkpoints (consumer_group TEXT PK,
                      last_global_sequence BIGINT,
                      updated_at TIMESTAMPTZ)

notifications (id BIGSERIAL PK, stream_id TEXT UNIQUE,
              notification_type TEXT, recipient_id TEXT,
              payload JSONB, status TEXT, retry_count INT,
              failure_reason TEXT, last_global_sequence BIGINT,
...
1. Start the full Component system (Testcontainers for all databases + RabbitMQ)
...
5. Notification module reacting to bank events
6. Stop the system
...
# Read store migrations (point JDBC_URL at the read database)
JDBC_URL=jdbc:postgresql://localhost:5432/readstore bb migrate-read

# Notification database migrations (point JDBC_URL at the notification database)
...
|---|---|---|---|
| Unit | `account_test`, `transfer_test`, `notification_test`, `decider_test`, `decider_kit_test`, `store_test`, `projection_kit_test`, `saga_test` | Pure domain logic, command handler wiring, store utilities, factory functions | No |
| Functional | `functional_test` | End-to-end lifecycle (open -> deposit -> withdraw -> projection) | Yes |
| Integration | `integration_test`, `store_integration_test`, `transfer_saga_test`, `projection_test` | Concurrency, idempotency, migrations, projection correctness, cross-account sagas | Yes (1x Postgres) |
| Async | `async_projection_test`, `outbox_test`, `consumer_test`, `component_test` | Outbox record/poll/publish, two-database projection, batch processing, poison event handling, inbox dedup, Component lifecycle | Yes (2x Postgres) |
| End-to-end | `async_integration_test`, `system_integration_test` | Full async pipeline: command -> outbox -> RabbitMQ -> projector -> read DB; full system with all modules | Yes (3x Postgres + RabbitMQ) |
...
**Pure domain, infrastructure boundary** -- The domain (`bank.account`, `bank.transfer`, `notification.notification`) contains no I/O, no timestamps, no sequence numbers. All infrastructure concerns (concurrency, idempotency, persistence) live in `es.decider` and `es.store`. This makes the domain trivially testable and portable.
...
**Inbox-based exactly-once consumers** -- The consumer framework (`es.consumer`) uses `INSERT ON CONFLICT DO NOTHING` into a `consumer_inbox` table for deduplication. Multiple competing workers on the same queue can safely process events without double-processing. Combined with catch-up timers, this provides exactly-once semantics with disaster recovery.

**Integration event contracts** -- Shared Malli schemas in `events.bank` define the stable contract between producer (bank module) and consumers (notification module). Domain events are mapped to integration events via `bank.integration-events`, which filters and transforms before outbox publishing.
...
**Framework/domain separation** -- All infrastructure (`es.*`) is domain-agnostic. The bank and notification domains are consumers. Adding a new domain means writing new deciders, projections, and a system assembly -- the framework doesn't change.

**Module independence via integration events** -- The notification module depends only on the shared `events.bank` contract, not on any `bank.*` namespace. Modules communicate through the event store and RabbitMQ, not direct function calls. This enables independent deployment, testing, and evolution.

## Layered Architecture

The codebase uses a **three-layer split** -- framework (`es.*`), domain modules (`bank.*`, `notification.*`, `events.*`), and composition root (`system`) -- rather than a deeper layered architecture like hexagonal/onion/DDD layers. This section explains what we have, what we considered, and why we stopped here.

### Current dependency structure

```
system          (root)  --depends-on-->  bank.*, notification.*, es.*
bank.*          (domain) --depends-on-->  es.*, events.bank
notification.*  (domain) --depends-on-->  es.*, events.bank
events.bank     (contract) --depends-on-->  (nothing -- pure schemas)
es.*            (framework) --depends-on-->  (external libs only)
  ...
  bank.integration-events ------->  events.bank
  bank.system ------------------->  es.projection, es.projection-kit
  bank.components --------------->  es.component, es.projection-kit
  notification.notification ----->  es.decider-kit, es.schema
  notification.reactor ---------->  events.bank, es.decider
  notification.notification-projection -> es.projection-kit, next.jdbc
  notification.component -------->  es.consumer, es.component
```

Key properties:

- **`es.*` never imports `bank.*` or `notification.*`** -- the framework is completely domain-agnostic. Zero circular dependencies.
- **Domain aggregates are pure** -- `bank.account`, `bank.transfer`, and `notification.notification` import only `es.decider-kit` and `es.schema`. No I/O, no JDBC, no database. They're portable and trivially testable.
- **Modules communicate via contracts** -- `notification.reactor` depends on `events.bank` (shared schemas), not on `bank.account` or `bank.transfer`. The bank module doesn't know the notification module exists.
- **Projection handlers import `next.jdbc`** -- `bank.account-projection`, `bank.transfer-projection`, and `notification.notification-projection` use JDBC directly in their SQL handlers. This is the one place domain code touches infrastructure.
...
|---|---|---|
| Pure domain | `bank.account`, `bank.transfer`, `notification.notification` | None -- no I/O, no DB |
| Contracts | `events.bank` | None -- pure Malli schemas |
| Command handling | `es.decider`, `es.store` | JDBC, advisory locks |
| Projections | `bank.*-projection`, `notification.*-projection`, `es.projection*` | JDBC |
| Messaging | `es.outbox`, `es.rabbitmq`, `es.consumer`, `es.async-projection` | JDBC, Langohr |
| Reactors | `notification.reactor` | es.decider (issues commands) |
| Lifecycle | `es.component`, `es.infra` | Component, Testcontainers |
| Composition | `system`, `bank.components`, `notification.component` | Component |
...
**~30 namespaces don't need onion layers.** The overhead of defining protocols, adapters, and wiring for a codebase this size would add indirection without solving a real problem. You can read the entire dependency graph above and verify correctness by inspection.
...
- **Many more modules** -- if the number of domain modules grows significantly, a more formal module registry or service discovery pattern may be warranted.

For now, the discipline of "aggregates are pure, projections are SQL, modules communicate via integration event contracts, framework doesn't know about domain" gives the important benefits without the ceremony.
...
- Jerome Chassaing -- [Functional Event Sourcing: Decider](https://thinkbeforecoding.com/post/2021/12/17/functional-event-sourcing-decider) -- the pattern this project implements
