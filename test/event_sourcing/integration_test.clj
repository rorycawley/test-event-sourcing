(ns event-sourcing.integration-test
...
(deftest append-events-requires-explicit-command-metadata
  (let [append-events (resolve 'event-sourcing.store/append-events!)]
...
(deftest projection-processes-events-and-balance
  ...
  (is (= 3 (projection/process-new-events! support/*ds*)))
  (is (= 0 (projection/process-new-events! support/*ds*)))

  (let [balance-row (projection/get-balance support/*ds* "acct-42")]
         ...
         (store/append-events! support/*ds*
                               ...
                               [{:event-type    "account-opened"
                                ...
                                {:event-type    "money-deposited"
                                 :event-version 1
                                 :payload       {:amount 10}}])))
  (is (= 2 (projection/process-new-events! support/*ds*)))
  (let [balance-row (projection/get-balance support/*ds* "legacy-upcast-1")]
                               ...
                               [{:event-type    "account-opened"
                                ...
                                {:event-type    "money-deposited"
                                 ...
                                 :payload       {:amount 7
                                                 :origin "legacy"}}])))
  (is (= 2 (projection/process-new-events! support/*ds*)))
  (let [balance-row (projection/get-balance support/*ds* "legacy-upcast-2")]
...
(deftest projection-fail-fast-does-not-advance-checkpoint
  ...
  (is (nil? (projection/get-balance support/*ds* "broken-1"))))
...
(deftest projection-invalid-event-shape-fails-and-keeps-checkpoint
  ...
  (is (= 0 (support/checkpoint)))
  (is (nil? (projection/get-balance support/*ds* "broken-2"))))
...
(deftest concurrent-projection-workers-are-serialized
  ...
  (let [results (sort (run-concurrently
                       [(fn [] (projection/process-new-events! support/*ds*))
                        (fn [] (projection/process-new-events! support/*ds*))]))]
    (is (= [0 2] results))
    (is (= 2 (support/checkpoint)))
    (is (= 5 (:balance (projection/get-balance support/*ds* "proj-race-1"))))))
